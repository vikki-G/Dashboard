...
function bindAgentDashboardDetailList(agentList) {
	...
	for (var i = 0; i < agentList.length; i++) {
		...
		if (_jsonAccountList != null || _jsonAccountList != undefined) {
			...
			for (var j = 0; j < _jsonAccountList.length; j++) {
				...
				//if (_jsonAccountValues != null && _jsonAccountValues != undefined && _userDetails != undefined ) {
					...
					if(_userDetails.groups != undefined) {
					for(var a=0; a < _userDetails.groups.length; a++ ) {
						
						var _lteam = _userDetails == undefined ? 'Others' : _userDetails.groups[a];
						if(!_teamGroupList.includes(_lteam))
							_teamGroupList.push(_lteam);
						var accountDetails = {
							team: _userDetails == undefined ? 'Others' : _userDetails.groups[a],
		...
		var agentDetails = {
			...
			team: (_userDetails == undefined)? '-' : ((_userDetails.groups == undefined) ? '-' : _userDetails.groups.toString()),
...
function refreshSupervisorDashboardEvent() {
	...
	if (refreshSupervisorDashboard && dashboard == 'DASHBOARD_006') {
